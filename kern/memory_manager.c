...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	void *v;
	v=kmalloc(PAGE_SIZE);
	unsigned int pa=kheap_physical_address((unsigned int)v);
	//uint32 f_n=pa/PAGE_SIZE;
	//f_n=(f_n|(PERM_USER|PERM_WRITEABLE|PERM_PRESENT));
	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(pa, PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
	//change this "return" according to your answe
	uint32 *table_ptr;
	table_ptr=(uint32*)v;
	//get_page_table(ptr_page_directory,(void*)v,&table_ptr);
	int i=0;
	for(;i<1024;i++)
		table_ptr[i]=0;
	return v;
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 length=virtual_address+size;
	uint32 va=virtual_address;
	uint32 num_of_pages=size/PAGE_SIZE;
	//------------------------------------------------------------------------------------------
	for(;virtual_address<(length);virtual_address+=PAGE_SIZE)
	{
		//-----------------------------------------------------------------------
		if(pf_read_env_page(e,(void*)virtual_address)==E_PAGE_NOT_EXIST_IN_PF)
		{
			pf_add_empty_env_page(e,virtual_address,0);
		}
	}
	//------------------------------------------------------------------------------------------
	if(num_of_pages<=e->page_WS_max_size)
	{
		int c=0,i=0;
		//================================
		...
		if(c>=num_of_pages)
		{
			...
			//---------------------------------------------------
			for(i=0,c=0;i<e->page_WS_max_size;i++)
			{
				//=================================================================================
				if(env_page_ws_is_entry_empty(e,i)==1)
				{
					...
					map_frame(curenv->env_page_directory , frame, (void*)va, PERM_USER | PERM_WRITEABLE| PERM_PRESENT);
...
// [2] freeMem
uint32 tablesvirtual[(USER_HEAP_MAX-USER_HEAP_START)/(1024*4096)];
void freeMem(struct Env* e, uint32 va, uint32 size)
{
	...
	//-----------------------------------------------------------------------------
	uint32 length=ROUNDUP((va+size),PAGE_SIZE);
	va=ROUNDDOWN(va,PAGE_SIZE);

	int i=0,j=0;
	uint32 curr_table=PDX(va),last_table=PDX(length),table_empty=1;
	uint32 *page_table;
	//=================================================================================
	for(;va<length;va+=PAGE_SIZE)
	{
		/*[1]Free ALL pages of the given range from the Page File*/
		...
		{
			if(ROUNDDOWN(e->ptr_pageWorkingSet[i].virtual_address,PAGE_SIZE) == ROUNDDOWN(va,PAGE_SIZE))
			{
				//-------------------------------------------------------------------
	...
	for(;curr_table<=last_table;curr_table++)
	{
		if(get_page_table(e->env_page_directory,(void*)(curr_table<<22),&page_table)!=TABLE_NOT_EXIST)
		{
			for(j=0;j<1024;j++)
			{
				if(page_table[j]!=0)
				{
					table_empty=0;//table not empty
					break;
				}
			}
			//---------------------------
			if(table_empty==1)//that mean table is empty
			{
				//unmap_frame(e->env_page_directory,(void*)page_table);
				kfree((void*)page_table);
				pd_clear_page_dir_entry(e,(curr_table<<22));//delete table from page
			}
			table_empty=1;
		}
	}
...
void moveMem(struct Env* e, uint32 src_virtual_address, uint32 dst_virtual_address, uint32 size)
{
	...
	//-----------------------------------------------
	for(;src_virtual_address<(src_virtual_address+size);src_virtual_address+=PAGE_SIZE,dst_virtual_address+=PAGE_SIZE)
	{
