...
struct UserProgramInfo userPrograms[] = {
		{"tst_mod_2","test",PTR_START_OF(tst_mod_2)},
...
void * create_user_page_WS(unsigned int numOfElements)
{
	...
	//void *v;
	//v=kmalloc((sizeof(struct WorkingSetElement)) *numOfElements);
	//change this "return" according to your answer
	//return (void*)kmalloc(sizeof(numOfElements*(struct WorkingSetElement)));

	return kmalloc((sizeof(struct WorkingSetElement)) *numOfElements);
}
...
void env_free(struct Env *e)
{
	...
	//-------------------------------------------------------------------
	// [2] Free the PAGE working set array itself from the main memory
	//unmap_frame(e->env_page_directory,(void*)e->__uptr_pws);
	kfree((void*)e->ptr_pageWorkingSet);
	//-------------------------------------------------------------------
	// [3] Free all TABLES from the main memory
	uint32 address=0 , *table_ptr;
	uint32 top=PDX(USER_TOP);
	...
	for(i=0;i<top;i++)
	{
		...
		{
			pd_clear_page_dir_entry(e,address);
	...
	//-------------------------------------------------------------------
	// [4] Free the page DIRECTORY from the main memory
	//unmap_frame(ptr_page_directory,(void*)e->env_page_directory);
